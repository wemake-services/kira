defmodule KiraWeb.AuthControllerTest do
  ...
  @ueberauth_auth %{
    ...
    info: %{email: "john.snow@knewnothing.com", nickname: "john.snow", name: "John Show"},
    provider: :github,
    uid: 259250
  }
  ...
  test "creates user from github info", %{conn: conn} do
    ...
    assert json_response(conn, 200) == %{"user" => %{"access_level" => nil, "email" => "john.snow@knewnothing.com", "expires_at" => nil, "state" => "active", "uid" => 259250, "username" => "john.snow"}}
  end

  test "find and return user from github email", %{conn: conn} do
    user = insert(:user, %{email: "john.snow@knewnothing.com", expires_at: nil, access_level: nil})
